...
--------------------
Reads Peak waveforms (Peak-Wfm) from the EPICS IOC and plots
histograms with Gaussian fit overlays for TP1, TP2, TP3,
or all three in a 1x3 grid.

Usage:
    python3 peak_distribution.py                        # all three channels
    python3 peak_distribution.py --channel tp1          # single channel
    python3 peak_distribution.py --channel tp2 --bins 80
    python3 peak_distribution.py --prefix "LN-BI{ACMI2:A}"
"""
...
CHANNELS = {
    "tp1": ("TP1:peak-Wfm", "TP1 Peak"),
    "tp2": ("TP2:peak-Wfm", "TP2 Peak"),
    "tp3": ("TP3:peak-Wfm", "TP3 Peak"),
}
...
def draw_histogram(ax, data: np.ndarray, bins: int, label: str,
    ...
    ax.set_xlabel(f"Peak ({UNITS})", fontsize=10)
...
def plot_all(prefix: str, bins: int, timeout: float) -> None:
    ...
    fig.suptitle("Pulse Peak Distributions", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    out = "peak_dist_all.png"
    plt.savefig(out, dpi=150)
    print(f"Saved {out}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Peak-Wfm histogram with Gaussian fit")
